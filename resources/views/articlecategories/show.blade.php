...
@section('url'){{url('irasok')}}/{{$category->id}}/{{$category->slug}}@endsection
...
@section('content')
	<div class="inner_box narrow-page" style="margin-top:6px;">
		...
		@if (Auth::check() && Auth::user()->id==$category->user->id)
			<a href="{{url('iras-temakor')}}/{{$category->id}}/{{$category->slug}}/modosit" type="submit" class="btn btn-default"><i class="fa fa-edit" aria-hidden="true"> </i>Módosít</a>
